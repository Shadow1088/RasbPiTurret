...
while running:
    ...
    try:
        ...
        if face_x + face_width // 2 < center_x and is_center != True:
            currentx = currentx-5
            setAngle(servox,currentx,0.3)
        #rotate X if upperright
        elif face_x + face_width // 2 > center_x and is_center != True:
            currentx = currentx+5
            setAngle(servox,currentx,0.3)
        #rotate Y if lowerleft
        if face_y + face_width // 2 < center_y and is_center != True:
            currenty = currenty-5
            setAngle(servoy,currenty,0.3)
